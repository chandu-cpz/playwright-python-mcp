...
test('browser_console_messages', async ({ client, server }) => {
  ...
  expect(resource).toHaveResponse({
    result: expect.stringMatching(new RegExp(`\\[LOG\\] Hello, world! @ ${escapeRegExp(server.PREFIX)}/?:4`)),
  });
  expect(resource).toHaveResponse({
    result: expect.stringMatching(new RegExp(`\\[ERROR\\] Error @ ${escapeRegExp(server.PREFIX)}/?:5`)),
  });
});
