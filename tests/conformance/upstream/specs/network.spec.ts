...
test('browser_network_requests', async ({ client, server }) => {
...

  {
    ...
    expect(response.result).toMatch(new RegExp(String.raw`^\d+\. \[GET\] ${escapeRegExp(serverUrl(server.PREFIX, 'json'))} => \[200\] OK$`, 'm'));
    expect(response.result).toMatch(new RegExp(String.raw`^\d+\. \[GET\] ${escapeRegExp(serverUrl(server.PREFIX, 'image.png'))} => \[404\]`, 'm'));
...
test('browser_network_requests filter', async ({ client, server }) => {
...

  {
    ...
    expect(response.result).toContain(serverUrl(server.PREFIX, 'api/users'));
    expect(response.result).toContain(serverUrl(server.PREFIX, 'api/orders'));
    expect(response.result).not.toContain(serverUrl(server.PREFIX, 'static/image.png'));
  }
});
...
test('browser_network_request shows full request and response details', async ({ client, server }) => {
  ...
  expect(detail.result).toContain(`#${index} [POST] ${serverUrl(server.PREFIX, 'api')}`);
...
test('browser_network_request reports failed requests', async ({ client, server }) => {
  ...
  expect(detail!.result).toContain(`#${match![1]} [GET] ${serverUrl(server.PREFIX, 'missing.png')}`);
...
function serverUrl(prefix: string, path: string): string {
