...
test('browser_storage_state saves storage state to file', async ({ startClient, server }, testInfo) => {
  ...
  expect(result).toHaveResponse({
    ...
    code: expect.stringContaining('page.context.storage_state'),
  });
...
test('browser_set_storage_state restores storage state from file', async ({ startClient, server, mcpBrowser }, testInfo) => {
  ...
  expect(result).toHaveResponse({
    ...
    code: expect.stringContaining('page.context.set_storage_state'),
  });
...
test.skip('--storage-state option works with remote endpoint in config', async () => {
  // Requires remote endpoint and --storage-state CLI parity; tracked in Slice 15.
});
