...
for (const mode of ['isolated', 'persistent']) {
  test.skip(`should work with recordVideo (${mode})`, async ({ startClient, server }, testInfo) => {
...
test('reports missing ffmpeg, not missing browser, when recordVideo is enabled', {
  ...
  test.skip(true, 'Requires contextOptions/recordVideo CLI parity; tracked in Slice 15.');
